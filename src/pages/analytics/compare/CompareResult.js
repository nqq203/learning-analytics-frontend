...
const CompareResult = ({ data, mode, onBack, criteria, course }) => {
    if (!data) {
        return (
            <Card>
                <CardContent>
    ...
    return (
        <Card style={{ marginTop: "32px" }}>
            <CardContent>
                <Typography variant="h6" style={{ textAlign: "center" }}>Kết quả so sánh</Typography>
                 {/* Thông tin */}
                 <Typography variant="body1" style={{ marginTop: "16px" }}>
                    <strong>Tiêu chí: </strong>{criteria}
                </Typography>
                <Typography variant="body1">
                    <strong>Môn học: </strong>{course}
                </Typography>
                <Typography variant="body1">
                    <strong>{mode === "class" ? "Lớp" : "Khoá"}: </strong>{data?.[0]?.class_name || "N/A"}
                </Typography>
                ...
                <TableContainer component={Paper} style={{ marginTop: "16px" }}>
                    <Table>
                        <TableHead>
                            <TableRow>
                                <TableCell><strong>{mode === "class" ? "Tên lớp" : "Khoá"}</strong></TableCell>
                        ...
                        <TableBody>
                            {Object.values(data).map((item, index) => (
                                <TableRow key={index} style={{ backgroundColor: index % 2 === 0 ? "#f9f9f9" : "#fff" }}>
                                    ...
                                    {mode === "class" && <TableCell>{item.course_name}</TableCell>}
