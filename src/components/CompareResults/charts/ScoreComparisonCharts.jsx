...
export default function ScoreComparisonCharts({ chartType, data, getDisplayName, getClassColor,scoreType }) {
  if (!data || data.length === 0) {
    return <div className="text-center py-10">Không có dữ liệu để hiển thị.</div>
  }

  if (chartType === "bar") {
    return <BarChartComponent data={data} scoreType={scoreType} />
  }
